...
describe('plugin manifests', () => {
  ...
  test('ceo.md description is written to trigger proactively', () => {
    const content = fs.readFileSync(path.join(agentsSourceDir, 'c-level', 'ceo.md'), 'utf8');
...
// Source files may live in department subfolders (.claude/agents/technical/dev.md);
// the plugin copy always stays flat by basename — see build-plugin.mjs for why.
function describePluginSync(label, sourceSubdir, pluginSubdir) {
  ...
  describe(`plugin ${pluginSubdir}/ stays in sync with .claude/${sourceSubdir}/`, () => {
    ...
    const sourceBasenames = sourceFiles.map((f) => path.basename(f)).sort();

    test(`${pluginSubdir}/ has exactly one generated copy per ${label} in .claude/${sourceSubdir}/`, () => {
      assert.deepEqual(pluginFiles, sourceBasenames);
    });

    for (const file of sourceFiles) {
      const basename = path.basename(file);
      test(`${pluginSubdir}/${basename} matches "npm run build:plugin" output for .claude/${sourceSubdir}/${file}`, () => {
        const expected = toPluginText(path.join(sourceDir, file));
        const actual = fs.readFileSync(path.join(pluginDir, basename), 'utf8');
        assert.equal(actual, expected, `${pluginSubdir}/${basename} is stale — run "npm run build:plugin"`);
      });
    }
  });
}
