...
import { ALLOWED_TOOLS, ALLOWED_MODELS, parseAgentFile as parseAgentFileShared, listAgentFiles } from '../app/server/agents.js';
...
const POLICY = {
  ...
  'sales-engineer': { tools: 'Read, Grep, Glob, Write, Edit, WebSearch, WebFetch', model: 'sonnet' },
  'customer-success': { tools: 'Read, Grep, Glob, Write, Edit, WebSearch, WebFetch', model: 'sonnet' },
  'data-analyst': { tools: 'Read, Grep, Glob, Bash, WebSearch, WebFetch', model: 'sonnet' },
};
const ROSTER = Object.keys(POLICY).sort();

const parseAgentFile = parseAgentFileShared;

// Relative paths (e.g. "technical/dev.md") — files may live in department
// subfolders under agentsDir; a role's identity comes from its frontmatter
// `name`, not the path, so every lookup below resolves by basename.
const files = listAgentFiles(agentsDir);
const pathByName = Object.fromEntries(files.map((f) => [path.basename(f, '.md'), f]));
...
describe('roster-wide policy', () => {
  test('exactly the roster in the POLICY table exists, no more, no less', () => {
    const names = files.map((f) => path.basename(f, '.md')).sort();
  ...
  test('cfo, security-engineer, and support-engineer hold no Write/Edit', () => {
    for (const name of ['cfo', 'security-engineer', 'support-engineer']) {
      const { frontmatter } = parseAgentFile(path.join(agentsDir, pathByName[name]));
      const tools = frontmatter.tools.split(',').map((t) => t.trim());
  ...
  test('ceo and dev-lead are instructed to run autonomously to completion', () => {
    for (const name of ['ceo', 'dev-lead']) {
      const { body } = parseAgentFile(path.join(agentsDir, pathByName[name]));
  ...
  test('dev-lead caps the fix loop at 3 cycles before escalating', () => {
    const { body } = parseAgentFile(path.join(agentsDir, pathByName['dev-lead']));
