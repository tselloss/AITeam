...
- `sales-engineer` — pre-sales technical questions, non-binding feasibility/estimate scoping
- `customer-success` — post-sale account health, renewal risk, expansion/feature feedback
- `data-analyst` — usage/telemetry analysis for `cpo` and `cfo`
...
`ceo` routes prospect technical questions to `sales-engineer` (scoping memo → `docs/sales/`,
handing off to `cpo` for roadmap gaps or `dev-lead` once it's a committed task) and
existing-customer account questions to `customer-success` (account summary →
`docs/customers/`, handing off to `product-owner` for feature requests or `support-engineer`
for bug reports). `cpo` and `cfo` route usage/cost-data questions to `data-analyst`
(findings memo → `docs/intelligence/`). None of the three bypass the existing pipeline —
they feed into it at its normal entry points rather than shipping or deciding anything
themselves.
...
`ceo` and `dev-lead` drive long sequential `Agent` delegation chains, and a long chain is exactly where an agent's own context may be compacted or summarized mid-run. A compacted summary is not guaranteed to preserve the specifics of this protocol. If either role notices its own context has been compacted or summarized during a run — or is otherwise unsure it still holds the exact rules — it must re-ground itself by re-reading this file before issuing the next delegation, rather than trusting the summary. Re-verify in particular: the `HANDOFF` block format (§ Mechanics), the authority rules — ADR bindingness and the CFO/security/QA gates (§ Authority rules), the stop-and-report conditions (§ Autonomous execution), and the version-control mechanics (§ Version control). Re-grounding is cheap; drifting from a coordination rule mid-chain is not.

## Version control

- Every engineering task runs on its own feature branch cut from `main` (e.g.
  `feature/<short-slug>`), never directly on `main`.
- When `dev-lead` delegates an implementation task to `dev` via the `Agent` tool, it
  passes `isolation: "worktree"` so `dev`'s in-progress branch work happens in a
  separate git worktree instead of switching branches in the primary checkout. This
  keeps the primary working tree stable for anyone (human or process) inspecting it
  mid-task and ensures an abandoned or failed task leaves no partial edits in the main
  checkout. The branch/commit/push and PR-merge mechanics in this section are
  unchanged: `dev` still commits on its feature branch and pushes it to `origin`
  before handing back, and `dev-lead` still opens and merges the PR from the pushed
  branch — because the branch reaches `origin`, the merge is unaffected by which
  working tree `dev` used. This does not enable parallel fan-out (still one `dev` at a
  time per ADR-001); it isolates only the single active task's working tree. Binding
  per `docs/decisions/adr-002-dev-task-worktree-isolation.md`.
...
- Shared artifacts are the interface between roles, not conversational memory:
  ...
  - `docs/requirements/` — Business Analyst requirements docs
  - `docs/architecture/` and `CONTRIBUTING.md` — Documentation Engineer's architecture
    reference and contributor guide
  - `docs/sales/` — Sales Engineer scoping memos
  - `docs/customers/` — Customer Success account summaries
  - `docs/intelligence/` — Data Analyst findings memos
