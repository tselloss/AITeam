...
import { listProjects, getProject, createProject, setProjectRepo, recordRunStart, recordRunFinish, reconcileOrphanedRuns } from './projects.js';
import { startBuildRun } from './build-runner.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const appRoot = path.join(__dirname, '..');
const workspacesRoot = path.join(appRoot, 'workspaces');
// One append-only log file per run, so a run's step-by-step history
// survives a server restart even though the live `runs` Map (below)
// doesn't — the events written here are the same ones already streamed to
// the browser over SSE, nothing extra.
const runLogsRoot = path.join(appRoot, 'run-logs');
fs.mkdirSync(runLogsRoot, { recursive: true });

// A run's persisted record can be stuck at status: 'running' forever if the
// server died or was restarted mid-run. Sweep those into 'error' on startup
// so the UI reflects reality instead of quietly showing "idle".
...
function emit(run, event) {
  ...
  fs.appendFileSync(run.logPath, `${JSON.stringify({ ts: new Date().toISOString(), ...event })}\n`);
}
...
app.post('/api/runs', async (req, res) => {
  ...
  const logPath = path.join(runLogsRoot, `${runId}.log`);
...
app.get('/api/runs/:id/events', (req, res) => {
  ...
  // On a browser-initiated reconnect (network blip, tab woken from sleep —
  // anything short of the server itself forgetting this run), EventSource
  // resends the id of the last event it saw via Last-Event-ID. Replay only
  // what came after that instead of the whole buffer, or a reconnecting
  // client would see every earlier log line duplicated.
  ...
  for (let i = startIndex; i < run.events.length; i++) {
