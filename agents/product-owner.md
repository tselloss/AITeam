...
Backlog files written or updated under `docs/backlog/`, plus a short summary and a `HANDOFF` block naming the next role — tag `Area`/`Severity` when the handoff traces back to a triaged support request.
