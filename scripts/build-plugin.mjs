...
function buildPluginDir(repoRoot, sourceSubdir, targetSubdir) {
  ...
  // Source files may live in department subfolders (e.g. .claude/agents/technical/dev.md)
  // for local organization, but the plugin copy always stays flat: nesting the plugin
  // output would change Claude Code's scoped invocation from "aiteam:dev" to
  // "aiteam:technical:dev", a breaking change for anyone with the plugin installed.
  ...
  if (dupes.length > 0) {
    throw new Error(`duplicate agent file name(s) across .claude/${sourceSubdir}/ subfolders: ${[...new Set(dupes)].join(', ')}`);
  }

  for (const file of sourceFiles) {
    const text = toPluginText(path.join(sourceDir, file));
    fs.writeFileSync(path.join(targetDir, path.basename(file)), text);
  }

  const stale = fs
    .readdirSync(targetDir)
    .filter((f) => f.endsWith('.md') && !basenames.includes(f));
